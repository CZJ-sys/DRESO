...
import h5py
import copy
from model.problems.base import BaseTimeDataset, BaseDataset
...
class RichtmyerMeshkov(BaseTimeDataset):
    ...
    def _state_at(self, trajectory_index, time_index):
        state = (
            torch.from_numpy(
                self.reader["solution"][trajectory_index, time_index, 0:4]
            )
            .type(torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        inputs = self._state_at(trajectory_index, t1)
...
class CompressibleBase(BaseTimeDataset):
    ...
    def _state_at(self, trajectory_index, time_index):
        state = (
            torch.from_numpy(self.reader["data"][trajectory_index, time_index, 0:4])
            .type(torch.float32)
            .reshape(4, self.resolution, self.resolution)
        )
        state[3] = state[3] - self.mean_pressure
        state = (state - self.constants["mean"]) / self.constants["std"]

        if self.tracer:
            tracer = (
                torch.from_numpy(self.reader["data"][trajectory_index, time_index, 4:5])
                .type(torch.float32)
                .reshape(1, self.resolution, self.resolution)
            )
